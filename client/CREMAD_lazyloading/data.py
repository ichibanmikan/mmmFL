...
class DataSet(Dataset):
    def __init__(self, data_dir, device="cuda"):
        ...
        self.wav_files = \
            sorted([f for f in os.listdir(self.wav_dir) if f.endswith(".wav")])
        self.flv_files = \
            sorted([f for f in os.listdir(self.flv_dir) if f.endswith(".flv")])

        self.mobilenet = models.mobilenet_v2(weights=models.MobileNet_V2_Weights.DEFAULT).to(device)
        self.mobilenet.classifier = \
            torch.nn.Sequential(*list(self.mobilenet.classifier.children())[:-1])
        self.mobilenet.eval()
        self.device = device
        ...
        for wav_file, flv_file in zip(self.wav_files, self.flv_files):
            if os.path.splitext(wav_file)[0] != os.path.splitext(flv_file)[0]:
    ...
    def __getitem__(self, index):
        file_name = os.path.splitext(self.wav_files[index])[0]

        wav_file = os.path.join(self.wav_dir, f"{file_name}.wav")
        flv_file = os.path.join(self.flv_dir, f"{file_name}.flv")

        wav = self.extract_audio_features(wav_file).astype(np.float32)
        flv = self.extract_video_features(flv_file).astype(np.float32)
        
        emotion_code = file_name.split("_")[2]
        label = self.emotion_map.get(emotion_code, -1)
        return wav, flv, len(wav), len(flv), label
    ...
    def extract_audio_features(self, wav_path, n_mfcc=80, max_frames=600):
        ...
        if mfcc_features.shape[0] < max_frames: # pad
