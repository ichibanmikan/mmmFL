...
import os
import time
from STRESS.model import *
from STRESS.train_tools import *
from STRESS.data import *
...
class Trainer:
    ...
    def every_epoch_train(self):
        ...
        for dt, labels in self.train_loader: 
            dt = dt.to(self.device)
            labels = labels.to(self.device)
            data_time.update(time.time() - end)
            output = self.model(dt) # (bsz, 3, 200), (bsz, 3, 200)
            bsz = len(dt)
