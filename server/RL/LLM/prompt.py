...
11. Considering 10., each set of sub-rewards (sub-rewards 0 - 7) corresponding to the high-level task assignment policy should be initialized to 0. However, if the high-level task assignment policy decides to **exclude a reputable client(e.g., top 20) from participating in the current training round**, or decides that **a client should participate in a task that has already reached its convergence goal (Current-round accuracy >= Predefined accuracy targets)**, then each set of sub-rewards corresponding to the high-level task assignment policy should receive the maximum penalty (e.g., sub-rewards[0:8] = -5) to ensure the punishment is maximized.
...
13. In line with point 12, I hope to achieve balanced growth of my tasks. Therefore, I would like to set a threshold A, which can be between 50 and 100 rounds. When a certain client i continuously participates in a specific task j for A rounds (counting only the rounds in which it actually participated in the task), I want to gradually increase the penalty. The initial penalty can be sub_rewards[0:8] = -1, and then it should progressively increase as the number of consecutive participations in task j grows (e.g., sub_rewards[0:8] = p, where p < -1). If the client decides not to participate in any task during a round, it should be punished according to rule 11 as a priority. If client i switches from task j to task k, the participation count for task j should be reset to 0, and the decision should continue to be guided by the client's reputation and the accuracy gain for the current round to determine the reward. In summary, I do not want any client to continue participating in a single task for more than A rounds. Once this threshold is exceeded, the client should be forced to switch to another task.
14. The above 13 rules are only intended for handling extreme edge cases in the system. They should not constitute the entire reward function.
        """
        self.Purpose = """ 
