...
13. In line with point 12, I hope to achieve balanced growth of my tasks. **Store in the history the number of times each client has participated in each task. If, for a given client, the participation counts across all of their high-reputation tasks become imbalanced (for example, the variance of those counts exceeds a specified threshold), impose a heavier penalty—initially setting sub_rewards[0:8] = -1—and then progressively increase the penalty as the imbalance grows (i.e., as the variance rises), until the variance falls back below the threshold.**
14. The above 13 rules are only intended for handling extreme edge cases in the system. They should not constitute the entire reward function. Meanwhile please do not use training accuracy as the sole evaluation criterion; the rewards for each client in every training round should comprehensively consider multiple factors, and the rewards should vary between clients accordingly.
15. **Be sure to generate rewards for the bandwidth allocation policy decisions. When generating rewards for the bandwidth allocation strategy, you must comprehensively consider both task participation and participation time.**
        """
        self.Purpose = """ 
